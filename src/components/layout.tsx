...
export function ParentBottomNav() {
  ...
  return (
    <nav
      ...
      aria-label="التنقل السفلي للوالد"
    >
...
export function ParentSidebar() {
  ...
  const systemTabs = [{ href: '/settings', label: 'الإعدادات', icon: <SettingsIcon size={18} /> }];
  ...
  return (
    <aside
      className="hidden md:flex fixed right-0 top-0 bottom-0 w-[var(--ghrs-sidebar-width)] z-40 flex-col"
      style={{
        background: 'var(--ghrs-bg-card)',
        borderLeft: '1px solid var(--ghrs-border-default)',
      }}
      aria-label="القائمة الجانبية للوالد"
    >
      {/* Logo */}
      <div className="px-5 py-5 border-b" style={{ borderColor: 'var(--ghrs-border-default)' }}>
        <Link href="/dashboard" className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] mt-0.5" style={{ color: 'var(--ghrs-text-tertiary)' }}>
              إدارة العائلة
            </p>
          </div>
        </Link>
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto" aria-label="روابط التنقل">
        ...
        <div className="space-y-0.5 mb-4">
          {mainTabs.map((tab) => {
            const isActive =
              pathname === tab.href || (tab.href === '/dashboard' && pathname === '/');
            return (
              <Link
                key={tab.href}
                href={tab.href}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-around h-16 px-1">
        {tabs.map((tab) => {
          const isActive =
            tab.href === '/child-mode' ? pathname === '/child-mode' : pathname.startsWith(tab.href);
          return (
            <Link
              key={tab.href}
              href={tab.href}
...
export function EmptyState({
  ...
  return (
    <div className="ghrs-card flex flex-col items-center justify-center py-12 px-4 text-center">
      ...
      <h3 className="text-base font-bold mb-1.5" style={{ color: 'var(--ghrs-text-primary)' }}>
        {title}
      </h3>
      <p className="text-sm mb-5 max-w-md" style={{ color: 'var(--ghrs-text-secondary)' }}>
        {description}
      </p>
      {action}
    </div>
    ...
    >
      ...
      >
        <span className="text-xl">
          {type === 'success' ? <CheckIcon size={20} /> : <RejectIcon size={20} />}
        </span>
        <p
          className={`flex-1 font-semibold ${type === 'success' ? 'text-ghrs-green-700' : 'text-ghrs-red-700'}`}
        >
          {message}
        </p>
        <button
