        ...
        >
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  >
                    {isQuran ? (
                      <QuranIcon size={24} color="var(--ghrs-green-600)" />
                    ) : isDua ? (
                      <SparkleIcon size={24} color="var(--ghrs-amber-600)" />
                    ) : (
                      <BookIcon size={24} color="var(--ghrs-text-secondary)" />
                    )}
                  </div>
                  <div>
                    <h2 className="text-lg font-bold" style={{ color: 'var(--ghrs-text-primary)' }}>
                      {task.title}
                    </h2>
            ...
            <div className="p-6">
              ...
              {hasContent && (
                <div
                  className="mb-4 p-5 rounded-2xl text-right"
                  style={{
                    background: isQuran
                      ? 'linear-gradient(135deg, var(--ghrs-green-900), var(--ghrs-green-800))'
                      : 'linear-gradient(135deg, var(--ghrs-amber-900), var(--ghrs-amber-800))',
                ...
                >
                  <h3
                    ...
                    style={{ color: isQuran ? '#86efac' : '#fcd34d' }}
                  >
              ...
              <div
                ...
                style={{ background: 'var(--ghrs-bg-tertiary)' }}
              >
                <div className="flex items-center gap-2">
                  <StarIcon size={20} color="var(--ghrs-amber-600)" />
                  <span className="font-bold" style={{ color: 'var(--ghrs-amber-600)' }}>
                    {task.xp_reward} XP
                  </span>
                </div>
                {task.money_reward > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold" style={{ color: 'var(--ghrs-green-600)' }}>
                      {formatMoney(task.money_reward)}
                    </span>
                  </div>
                )}
              </div>
              ...
              >
                {isCompleted ? (
                  <>
                    <CheckIcon size={20} className="inline" /> تم الإنجاز!
                  </>
                ) : isPending ? (
                  <>
                    <ClockIcon size={20} className="inline" /> بانتظار موافقة الوالد
                  </>
