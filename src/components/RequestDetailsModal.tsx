...
const statusConfig: Record<
  string,
  { text: string; color: string; bg: string; icon: React.ReactNode }
> = {
  pending: {
    text: 'بانتظار الموافقة',
    color: 'var(--ghrs-amber-700)',
    bg: 'var(--ghrs-amber-50)',
    icon: <ClockIcon size={16} color="var(--ghrs-amber-700)" />,
  },
  approved: {
    text: 'تمت الموافقة',
                ...
                {request.money_spent != null && request.money_spent > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      ...
                      style={{ color: 'var(--ghrs-text-primary)' }}
                    >
                      {formatMoney(request.money_spent)}
                    </span>
                  </div>
                )}
                <div className="flex items-center gap-2">
