...
export default function IconPicker({ selectedIcon, onSelect, onClose }: IconPickerProps) {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-lg font-bold mb-4" style={{ color: 'var(--ghrs-text-primary)' }}>
          اختر أيقونة
        </h2>
        ...
        <div className="flex gap-1 mb-4 overflow-x-auto pb-2">
          <button
            ...
            style={{
              background:
                activeCategory === 'all' ? 'var(--ghrs-green-100)' : 'var(--ghrs-bg-tertiary)',
              color:
                activeCategory === 'all' ? 'var(--ghrs-green-700)' : 'var(--ghrs-text-secondary)',
            }}
          >
            الكل
          </button>
          {ICON_CATEGORIES.map((cat) => (
            <button
              ...
              style={{
                background:
                  activeCategory === cat.id ? 'var(--ghrs-green-100)' : 'var(--ghrs-bg-tertiary)',
                color:
          ...
          {filteredIcons.map((icon) => {
            ...
            return (
              <button
                ...
                style={{
                  background: isSelected ? 'var(--ghrs-green-100)' : 'transparent',
                  border: `2px solid ${isSelected ? 'var(--ghrs-green-500)' : 'transparent'}`,
                }}
              >
                <IconComp
                  size={20}
                  color={isSelected ? 'var(--ghrs-green-600)' : 'var(--ghrs-text-secondary)'}
                />
                <span
                  className="text-[10px] font-semibold"
                  style={{
                    color: isSelected ? 'var(--ghrs-green-700)' : 'var(--ghrs-text-tertiary)',
                  }}
                >
                  {icon.label}
                </span>
