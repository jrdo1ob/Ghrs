...
export async function POST(request: NextRequest) {
  try {
    ...
    const { gift_id, title, description, cost_xp, cost_money, icon, is_active } =
      await request.json();
    if (!gift_id) {
    ...
    if (gift.family_id !== member.family_id) {
      return NextResponse.json(
        { success: false, error: 'الهدية لا تنتمي لعائلتك' },
        { status: 403 }
      );
    }
    ...
    const { error: updateError } = await supabase.from('gifts').update(updates).eq('id', gift_id);
