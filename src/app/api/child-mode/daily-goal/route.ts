...
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof target_tasks !== 'number' || target_tasks < 1 || target_tasks > 20) {
      return NextResponse.json({ success: false, error: 'الهدف يجب أن يكون بين 1 و 20' }, { status: 400 })
    }
    ...
    if (!result || !result.success) {
      return NextResponse.json({ success: false, error: result?.message || 'حدث خطأ' }, { status: 500 })
    }
