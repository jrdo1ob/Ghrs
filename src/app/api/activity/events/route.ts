...
export async function POST(request: NextRequest) {
  try {
    ...
    const latestHistoryByCompletion = new Map<
      string,
      { completion_id: string; action: string; performed_by: string | null; created_at: string }
    >();
    ...
    for (const c of completions) {
      ...
      const eventType =
        latestHistory?.action === 'revoke' || latestHistory?.action === 'revoked'
          ? 'revoked'
          : latestHistory?.action || 'completed';

      const isAdmin =
        eventType === 'approved' || eventType === 'rejected' || eventType === 'revoked';
      const performer =
        isAdmin && latestHistory?.performed_by
      ...
      if (type_filter !== 'all') {
        if (type_filter === 'completed') {
          ...
          shouldInclude =
            type_filter ===
            (c.approved === null ? 'pending' : c.approved === true ? 'approved' : 'rejected');
        }
      }
    ...
    if (familyGiftIdsForActivity.length > 0) {
      ...
      if (redemptionIds.length > 0) {
        const { data } = await supabase
          .from('gift_activity')
          .select(
            'id, gift_redemption_id, action, performed_by, xp_amount, money_amount, reason, created_at'
          )
    ...
    const { data: giftMembers } =
      giftMemberIds.length > 0
    ...
    const giftPerformerIds = [
      ...new Set(
        giftActivities.map((g) => g.performed_by).filter((id): id is string => id !== null)
      ),
    ];
    const { data: giftPerformers } =
      giftPerformerIds.length > 0
