...
export async function POST(request: NextRequest) {
  try {
    ...
    // 6. Call reject_task_completion with verified member_id
    const { data, error } = await supabase.rpc('reject_task_completion', {
      p_completion_id: completion_id,
      p_rejected_by: session.member.member_id,
    });

    if (error) {
      console.error('[GHRS REJECT] RPC error:', error.message);
      return NextResponse.json({ success: false, error: 'حدث خطأ أثناء الرفض' }, { status: 500 });
    }

    // 7. Return success
    return NextResponse.json({
      success: true,
      message: 'تم رفض الإنجاز',
      data: data,
    });
  } catch (err) {
    console.error('[GHRS REJECT] Unexpected error:', err);
