...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!taskData.title) {
      return NextResponse.json({ success: false, error: 'اسم المهمة مطلوب' }, { status: 400 });
    }

    const supabase = createServiceRoleClient();

    // Validate assigned_to members belong to the session family
    const assignedTo = taskData.assigned_to ? taskData.assigned_to : null;
    ...
    const { data: newTask, error: insertError } = await supabase
      ...
      .insert({
        ...
        requires_approval:
          taskData.requires_approval !== undefined ? taskData.requires_approval : true,
        is_active: true,
        created_by: memberId,
