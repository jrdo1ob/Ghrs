...
export default function ChildrenPage() {
  ...
  const [manualModal, setManualModal] = useState<{ child: any; type: 'reward' | 'penalty' } | null>(
    null
  );
  const [manualForm, setManualForm] = useState({
    reason: '',
    currencyType: 'xp' as 'xp' | 'money',
    amount: 10,
  });
        ...
        >
          ...
          >
            ...
            <div className="space-y-3">
              <label className="text-xs font-bold" style={{ color: 'var(--ghrs-text-secondary)' }}>
                عدد المهام
              </label>
              <input
                type="number"
                ...
                className="ghrs-input w-full text-center text-lg font-bold tabular-nums"
              />
            </div>
            ...
            <div className="flex gap-2 mt-5">
              <button
                ...
                disabled={processingId === goalModal.childId}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-all"
                style={{
                  background: 'var(--ghrs-amber-500)',
                  color: 'white',
                  opacity: processingId === goalModal.childId ? 0.6 : 1,
                }}
              >
                🎯 حفظ الهدف
              </button>
              <button onClick={() => setGoalModal(null)} className="ghrs-btn-secondary">
                إلغاء
              </button>
            </div>
          </div>
        ...
        >
          ...
          >
            <h2
              ...
              style={{
                color:
                  manualModal.type === 'reward' ? 'var(--ghrs-green-600)' : 'var(--ghrs-red-600)',
              }}
            >
              {manualModal.type === 'reward' ? 'مكافأة فورية' : 'خصم / عقاب'}
            </h2>
            <p className="text-sm mb-4" style={{ color: 'var(--ghrs-text-secondary)' }}>
              {manualModal.type === 'reward' ? 'منح مكافأة لـ' : 'تطبيق خصم على'}{' '}
              {manualModal.child.name}
            </p>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    manualModal.type === 'reward'
                      ? 'مساعدة الجدة، خلق حسن، تميز في الاختبار'
                      : 'عدم الالتزام، سلوك غير لائق، صراخ'
                  }
                />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => setManualForm({ ...manualForm, currencyType: 'xp' })}
                    ...
                    style={{
                      ...
                      border: `1px solid ${manualForm.currencyType === 'xp' ? 'var(--ghrs-green-300)' : 'var(--ghrs-border-default)'}`,
                    }}
                  >
                    <StarIcon size={14} /> XP
                  </button>
                  <button
                    type="button"
                    onClick={() => setManualForm({ ...manualForm, currencyType: 'money' })}
                    className="flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg text-sm font-bold transition-all"
                    style={{
                      background:
                        manualForm.currencyType === 'money'
                          ? 'var(--ghrs-amber-50)'
                          : 'var(--ghrs-bg-secondary)',
                      color:
                        manualForm.currencyType === 'money'
                          ? 'var(--ghrs-amber-700)'
                          : 'var(--ghrs-text-secondary)',
                      border: `1px solid ${manualForm.currencyType === 'money' ? 'var(--ghrs-amber-300)' : 'var(--ghrs-border-default)'}`,
                    }}
                  >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setManualForm({ ...manualForm, amount: parseFloat(e.target.value) || 0 })
                  }
                  className="ghrs-input w-full text-center text-lg font-bold tabular-nums"
                />
              </div>
            ...
            <div className="flex gap-2 mt-5">
              <button
                ...
                disabled={
                  ...
                  manualForm.amount <= 0 ||
                  processingId === manualModal.child.id
                }
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-all"
                style={{
                  background:
                    manualModal.type === 'reward' ? 'var(--ghrs-green-500)' : 'var(--ghrs-red-500)',
                  color: 'white',
                  opacity:
                    !manualForm.reason.trim() ||
                    manualForm.amount <= 0 ||
              ...
              >
                {manualModal.type === 'reward' ? (
                  <>
                    <SparkleIcon size={14} /> منح المكافأة
                  </>
                ) : (
                  <>
                    <ShieldIcon size={14} /> تطبيق الخصم
                  </>
                )}
              </button>
              <button onClick={() => setManualModal(null)} className="ghrs-btn-secondary">
                إلغاء
              </button>
            </div>
          </div>
        ...
        <div className="p-4 md:p-8 max-w-4xl mx-auto">
          ...
          >
            <button
              ...
              style={{
                borderColor: activeTab === 'children' ? 'var(--ghrs-green-600)' : 'transparent',
                color:
                  activeTab === 'children'
            ...
            <button
              ...
              className="px-4 py-2.5 text-sm font-bold transition-all border-b-2"
              style={{
                borderColor: activeTab === 'parents' ? 'var(--ghrs-green-600)' : 'transparent',
                color:
                  activeTab === 'parents'
                    ? 'var(--ghrs-text-primary)'
                    : 'var(--ghrs-text-tertiary)',
              ...
              <form onSubmit={handleAdd} className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder={activeTab === 'children' ? 'سارة' : 'شمه'}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    required
                    className="ghrs-input text-center text-lg tracking-widest font-mono"
                    placeholder="••••"
                    maxLength={6}
                    dir="ltr"
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-3 gap-2">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        >
                          {member.role === 'child' ? (
                            <LeafIcon size={20} color="var(--ghrs-green-600)" />
                          ) : member.role === 'owner' ? (
                            <CrownIcon size={20} color="var(--ghrs-amber-600)" />
                          ) : (
                            <MotherIcon size={20} color="var(--ghrs-blue-600)" />
                          )}
                        </div>
                    ...
                    {member.role === 'child' && (
                      <div className="flex gap-2 mb-3">
                        <button
                          onClick={() => {
                            setManualModal({ child: member, type: 'reward' });
                            setManualForm({ reason: '', currencyType: 'xp', amount: 10 });
                          }}
                        ...
                        <button
                          onClick={() => {
                            setManualModal({ child: member, type: 'penalty' });
                            setManualForm({ reason: '', currencyType: 'xp', amount: 10 });
                          }}
                          className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition-all"
                          style={{
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className="text-lg font-bold font-mono tracking-widest px-3 py-1.5 rounded-lg flex-1"
                          style={{
                          ...
                          style={{
                            ...
                            color:
                              copiedId === member.login_code
                                ? 'white'
                                : 'var(--ghrs-text-secondary)',
                            border: `1px solid ${copiedId === member.login_code ? 'var(--ghrs-green-500)' : 'var(--ghrs-border-default)'}`,
                          }}
                        >
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => copyLoginLink(member.login_code)}
                      ...
                      {member.role === 'child' && (
                        <button
                          onClick={() => {
                            const link = `${window.location.origin}/family-login?code=${member.login_code}`;
                            window.open(
                              `https://wa.me/?text=${encodeURIComponent(`بطلنا المبدع!\nحديقتك في منصة غرس بانتظارك اليوم!\n\nرابط دخولك المباشر:\n${link}`)}`,
                              '_blank'
                            );
                          }}
                          className="flex-1 px-3 py-2 rounded-lg text-xs font-bold transition-colors"
          ...
          {currentList.length === 0 && !showAdd && (
            <EmptyState
              icon={activeTab === 'children' ? <ChildIcon size={32} /> : <UserIcon size={32} />}
              title={`لم تتم إضافة أي ${emptyLabel} بعد`}
              description={
                activeTab === 'children'
                  ? 'أضف أطفالك لبدء مغامرة النمو'
                  : 'أضف أهل العائلة لإعطائهم صلاحيات الإدارة'
              }
              action={
