    ...
    <div className="min-h-screen" style={{ background: 'var(--ghrs-bg-primary)' }}>
      ...
      <div className="md:mr-[var(--ghrs-sidebar-width)] pb-24 md:pb-8">
        <div className="p-4 md:p-8 max-w-4xl mx-auto">
          <PageHeader
            title="القصص التربوية"
            subtitle="مكتبة القصص وإنشاء قصص مخصصة"
            backHref="/tasks"
            action={
              <div className="flex gap-2">
                <button
                  onClick={() => setActiveTab(activeTab === 'custom' ? 'library' : 'custom')}
                  className="ghrs-btn-primary"
                >
                  {activeTab === 'custom' ? (
                    <>
                      <BookIcon size={16} className="inline" /> المكتبة
                    </>
                  ) : (
                    <>
                      <EditIcon size={16} className="inline" /> قصة جديدة
                    </>
                  )}
                </button>
              </div>
            ...
            <div className="mb-4 flex items-center gap-2 flex-wrap">
              ...
              <div className="flex gap-1.5 flex-wrap">
                <button
                  ...
                  style={{
                    background: !selectedChild ? 'var(--ghrs-bg-secondary)' : 'transparent',
                    color: !selectedChild
                      ? 'var(--ghrs-text-primary)'
                      : 'var(--ghrs-text-tertiary)',
                    border: `1px solid ${!selectedChild ? 'var(--ghrs-border-default)' : 'transparent'}`,
                  }}
                >
                  الجميع
                </button>
                {children.map((c) => (
                  <button
                    ...
                    style={{
                      background:
                        selectedChild === c.id ? 'var(--ghrs-bg-secondary)' : 'transparent',
                      color:
                        selectedChild === c.id
                          ? 'var(--ghrs-text-primary)'
                          : 'var(--ghrs-text-tertiary)',
                      border: `1px solid ${selectedChild === c.id ? 'var(--ghrs-border-default)' : 'transparent'}`,
                    }}
                  >
                    <ChildIcon size={12} className="inline" /> {c.name}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2.5">
              {presetStories.length === 0 ? (
                <EmptyState
                  ...
                  title="لا توجد قصص في المكتبة"
                  description="جاري تحميل القصص..."
                />
              ) : (
                  ...
                  <div key={preset.id} className="ghrs-card p-4">
                    <div className="flex justify-between items-start gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs mb-2" style={{ color: 'var(--ghrs-text-secondary)' }}>
                          {preset.content.substring(0, 150)}...
                        </p>
                        <div className="flex gap-1.5 flex-wrap">
                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold ghrs-badge ghrs-badge-success">
                            <SparkleIcon size={10} /> {preset.moral_value}
                          </span>
                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold ghrs-badge ghrs-badge-warning">
                            <StarIcon size={10} /> 5 XP
                          </span>
                        </div>
                      </div>
                      <button
                        onClick={() => handleAddPreset(preset)}
                        className="ghrs-btn-primary text-xs py-2 px-3 flex-shrink-0"
                      >
          ...
          {activeTab === 'custom' && (
            <div className="ghrs-card p-5 ghrs-animate-scale-in">
              <h2
                ...
                style={{ color: 'var(--ghrs-text-primary)' }}
              >
                <EditIcon size={18} className="inline" /> إنشاء قصة جديدة
              </h2>
              <form onSubmit={handleCreateStory} className="space-y-3">
                <div>
                  <label className="ghrs-label">عنوان القصة *</label>
                  <input
                    type="text"
                    ...
                    placeholder="النحلة والوردة"
                  />
                </div>
                <div>
                  <label className="ghrs-label">نص القصة *</label>
                  <textarea
                    ...
                    className="ghrs-input w-full"
                    rows={6}
                    placeholder="كانت هناك نحلة صغيرة..."
                  />
                </div>

                <div>
                  <label className="ghrs-label">القيمة التربوية</label>
                  <div className="flex flex-wrap gap-1.5">
                    {MORAL_VALUES.map((v) => (
                      <button
                        key={v}
                        type="button"
                        onClick={() => setCustomForm({ ...customForm, moral_value: v })}
                        className="px-3 py-1.5 rounded-md text-xs font-bold transition-all"
                        style={{
                          background:
                            customForm.moral_value === v
                              ? 'var(--ghrs-green-50)'
                          ...
                          border: `1px solid ${customForm.moral_value === v ? 'var(--ghrs-green-300)' : 'var(--ghrs-border-default)'}`,
                        }}
                      >
                        {v}
                      </button>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="number"
                      value={customForm.reward_xp}
                      onChange={(e) =>
                        setCustomForm({ ...customForm, reward_xp: parseInt(e.target.value) || 5 })
                      }
                      min="1"
                      className="ghrs-input w-full"
                    />
                  ...
                  <div>
                    ...
                    <select
                      value={customForm.assigned_to}
                      onChange={(e) =>
                        setCustomForm({ ...customForm, assigned_to: e.target.value })
                      }
                      className="ghrs-input w-full"
                    >
                ...
                <div className="flex gap-2 pt-1">
                  <button type="submit" className="ghrs-btn-primary">
                    <BookIcon size={14} /> إنشاء القصة وتعيينها
                  </button>
                  <button
                    type="button"
            ...
            <div className="space-y-4">
              {stories.length === 0 ? (
                <EmptyState
                  ...
                  description="أضف قصة من المكتبة أو أنشئ قصة مخصصة"
                  action={
                    <button onClick={() => setActiveTab('library')} className="ghrs-btn-primary">
                      <BookIcon size={16} className="inline" /> تصفح المكتبة
                    </button>
                  }
                />
              ) : (
                  ...
                  <div key={story.id} className="ghrs-card p-5">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-lg">
                            <BookIcon size={20} className="inline" />
                          </span>
                          <h3
                        ...
                        <p
                          ...
                          style={{ color: 'var(--ghrs-text-secondary)', lineHeight: '1.8' }}
                        >
                        ...
                        <div className="flex gap-2">
                          {story.moral_value && (
                            ...
                            >
                              <SparkleIcon size={16} className="inline" /> {story.moral_value}
                            </span>
                          )}
                          <span
                            className="px-2 py-0.5 rounded-full text-xs font-bold"
                          ...
                          >
                            <StarIcon size={16} className="inline" /> {story.reward_xp} XP
                          </span>
                          {story.assigned_to && (
                            <span
                              className="px-2 py-0.5 rounded-full text-xs font-bold"
                            ...
                            >
                              <ChildIcon size={16} className="inline" />{' '}
                              {children.find((c) => c.id === story.assigned_to)?.name || '—'}
                            </span>
                          )}
                          {!story.assigned_to && (
                            <span
                      ...
                      <button
                        ...
                        className="p-2 rounded-lg transition-all hover:bg-ghrs-bg-tertiary"
                        style={{ color: 'var(--ghrs-red-500)' }}
                      >
                        <DeleteIcon size={16} className="inline" />
                      </button>
                    </div>
                  </div>
                ))
