...
export default function PresetTasksPage() {
  ...
  useEffect(() => {
    const getPresets = async () => {
      ...
      const { data } = await supabase.from('preset_tasks').select('*').order('sort_order');
        ...
        <div className="md:mr-[var(--ghrs-sidebar-width)] pb-24 md:pb-8 p-4 md:p-8">
          <div className="max-w-4xl mx-auto">
            <div className="animate-pulse space-y-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="ghrs-card h-24" />
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {filtered.map((preset) => (
              <div key={preset.id} className="ghrs-card p-4">
                ...
                <div
                  className="flex items-center gap-2 mb-3 pb-3 border-t pt-3 text-xs"
                  style={{ borderColor: 'var(--ghrs-border-default)' }}
                >
                  <span
                  ...
                  <span
                    className={`px-1.5 py-0.5 rounded text-[10px] font-bold ghrs-badge ${preset.requires_approval ? 'ghrs-badge-warning' : 'ghrs-badge-success'}`}
                  >
                ...
                >
                  {adding === preset.id ? (
                    <>
                      <ClockIcon size={12} className="inline" /> جاري...
                    </>
                  ) : (
                    '+ اضافة للعائلة'
                  )}
