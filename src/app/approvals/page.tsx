          ...
          >
            {[
              { key: 'all', label: 'الكل', count: pendingCount },
              {
                key: 'task',
                label: 'المهام',
                count: approvals.filter((a) => a.type === 'task' && a.status === 'pending').length,
              },
              {
                key: 'gift',
                label: 'الهدايا',
                count: approvals.filter((a) => a.type === 'gift' && a.status === 'pending').length,
              },
              {
                key: 'withdrawal',
                label: 'الأموال',
                count: approvals.filter((a) => a.type === 'withdrawal' && a.status === 'pending')
                  .length,
              },
            ].map((tab) => (
              <button
              ...
              >
                {tab.label}
                {tab.count > 0 && (
                  <span className="px-1.5 py-0.5 rounded-full text-[10px] font-bold tabular-nums ghrs-badge ghrs-badge-warning">
                    {tab.count}
                  </span>
                )}
              </button>
                    ...
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      <div
                        ...
                        style={{
                          ...
                          border: `1px solid ${item.type === 'task' ? 'var(--ghrs-amber-200)' : item.type === 'gift' ? 'var(--ghrs-green-200)' : 'var(--ghrs-border-default'}`,
                        }}
                      >
                        {item.type === 'task' && (
                          <TasksIcon size={16} color="var(--ghrs-amber-600)" />
                        )}
                        {item.type === 'gift' && (
                          <GiftsIcon size={16} color="var(--ghrs-green-600)" />
                        )}
                        {item.type === 'withdrawal' && (
                          <CoinIcon size={16} color="var(--ghrs-blue-600)" />
                        )}
                      </div>
