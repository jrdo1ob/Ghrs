-- ============================================================
-- Migration 049: Fix money penalty calculation bug
-- ============================================================
-- BUG: When money balance < requested penalty, the function
--      incorrectly uses XP balance (v_new_xp) instead of
--      money balance (v_new_money) as the deduction amount.
--
-- FIX: Change v_money_amount := v_new_xp to v_money_amount := v_new_money
--
-- SCOPE: Single line change in apply_manual_adjustment.
-- ============================================================
  ...
  ELSE -- penalty
    ...
    ELSE
      IF v_new_money < p_amount THEN
        v_money_amount := v_new_money;  -- FIX: was v_new_xp (bug)
      ELSE
