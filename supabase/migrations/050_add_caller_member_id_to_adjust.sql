-- ============================================================
-- Migration 050: Add p_caller_member_id to apply_manual_adjustment
-- ============================================================
-- The function relies on get_current_member_id() which uses auth.uid().
-- For Code + PIN users, auth.uid() is NULL, causing "غير مصرح" error.
--
-- FIX: Add optional p_caller_member_id parameter.
-- When provided by the API (after session validation), use it directly.
-- When NULL, fall back to get_current_member_id() for backward compatibility.
--
-- All existing authorization checks are PRESERVED:
-- - caller must exist in members
-- - caller must have parent/owner role
-- - target child must exist
-- - target child must belong to caller's family
-- ============================================================
